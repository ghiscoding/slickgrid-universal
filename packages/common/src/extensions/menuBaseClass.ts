  ...
  repositionMenu(
    ...
    if (menuElm && parentElm) {
      ...
      if (
        ...
        const viewportWidth = gridPos?.width || browserWidth;
        const requestedDropSide = (addonOptions as CellMenu | ContextMenu)?.dropSide;
        const dropSide = subMenuPosCalc >= viewportWidth ? 'left' : 'right';
        ...
        if (needHeaderMenuOffsetLeftRecalc) {
          ...
          if ((addonOptions as HeaderMenu)?.autoAlign && menuOffsetLeft + (menuElm.clientWidth ?? 0) >= viewportWidth) {
            menuOffsetLeft =
