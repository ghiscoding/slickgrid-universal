...
export class SlickCheckboxSelectColumn<T = any> {
  ...
  init(grid: SlickGrid): void {
    ...
    if (this.gridOptions.preselectedRows && this._rowSelectionModel && this._grid.getSelectionModel()) {
      window.setTimeout(() => this.selectRows(this.gridOptions.preselectedRows || []));
    }
  ...
  deSelectRows(rowArray: number[]): void {
    ...
    this._grid.setSelectedRows(
      this._grid.getSelectedRows().filter((n) => removeRows.indexOf(n) < 0),
  ...
  protected checkboxSelectionFormatter(
    row: number,
    _cell: number,
    _val: any,
    _columnDef: Column,
