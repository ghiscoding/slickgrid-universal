        ...
        if (columnDef && !columnDef.excludeFromHeaderMenu) {
          ...
          if (options.enableSorting && columnDef.sortable && headerMenuOptions && !headerMenuOptions.hideSortCommands) {
            if (columnHeaderMenuItems.filter((item: MenuCommandItem) => item.hasOwnProperty('command') && item.command === 'sort-asc').length === 0) {
              columnHeaderMenuItems.push({
                ...
                title: options.enableTranslate ? this.translaterService.translate('SORT_ASCENDING') : this._locales && this._locales.TEXT_SORT_ASCENDING,
                command: 'sort-asc',
                positionOrder: 50
              });
            }
            if (columnHeaderMenuItems.filter((item: MenuCommandItem) => item.hasOwnProperty('command') && item.command === 'sort-desc').length === 0) {
              columnHeaderMenuItems.push({
                ...
                title: options.enableTranslate ? this.translaterService.translate('SORT_DESCENDING') : this._locales && this._locales.TEXT_SORT_DESCENDING,
                command: 'sort-desc',
                positionOrder: 51
              });
            }
            ...
            if (!headerMenuOptions.hideClearSortCommand && columnHeaderMenuItems.filter((item: MenuCommandItem) => item.hasOwnProperty('command') && item.command === 'clear-sort').length === 0) {
              columnHeaderMenuItems.push({
                ...
                title: options.enableTranslate ? this.translaterService.translate('REMOVE_SORT') : this._locales && this._locales.TEXT_REMOVE_SORT,
                command: 'clear-sort',
                positionOrder: 54
              });
            }
          ...
          if (options.enableFiltering && columnDef.filterable && headerMenuOptions && !headerMenuOptions.hideFilterCommands) {
            if (!headerMenuOptions.hideClearFilterCommand && columnHeaderMenuItems.filter((item: MenuCommandItem) => item.hasOwnProperty('command') && item.command === 'clear-filter').length === 0) {
              columnHeaderMenuItems.push({
                ...
                title: options.enableTranslate ? this.translaterService.translate('REMOVE_FILTER') : this._locales && this._locales.TEXT_REMOVE_FILTER,
          ...
          if (headerMenuOptions && !headerMenuOptions.hideColumnHideCommand && columnHeaderMenuItems.filter((item: MenuCommandItem) => item.hasOwnProperty('command') && item.command === 'hide').length === 0) {
            columnHeaderMenuItems.push({
              ...
              title: options.enableTranslate ? this.translaterService.translate('HIDE_COLUMN') : this._locales && this._locales.TEXT_HIDE_COLUMN,
              command: 'hide',
              positionOrder: 55
            });
          }
        ...
        if (!columnDef.excludeFromHeaderMenu) {
          ...
          columnHeaderMenuItems.forEach((item: MenuCommandItem) => {
            if (item.hasOwnProperty('command')) {
              switch (item.command) {
                case 'clear-filter':
                  item.title = this.translaterService.translate('REMOVE_FILTER') || this._locales && this._locales.TEXT_REMOVE_FILTER;
                  break;
                case 'clear-sort':
                  item.title = this.translaterService.translate('REMOVE_SORT') || this._locales && this._locales.TEXT_REMOVE_SORT;
                  break;
                case 'sort-asc':
                  item.title = this.translaterService.translate('SORT_ASCENDING') || this._locales && this._locales.TEXT_SORT_ASCENDING;
                  break;
                case 'sort-desc':
                  item.title = this.translaterService.translate('SORT_DESCENDING') || this._locales && this._locales.TEXT_SORT_DESCENDING;
                  break;
                case 'hide':
                  item.title = this.translaterService.translate('HIDE_COLUMN') || this._locales && this._locales.TEXT_HIDE_COLUMN;
                  break;
              }
            }
