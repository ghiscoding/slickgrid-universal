...
export const iconBooleanFormatter: Formatter = (_row, _cell, value, columnDef) => {
  ...
  if (!cssClasses) {
    throw new Error(
      '[Slickgrid-Universal] When using `Formatters.iconBoolean`, you must provide You must provide the "cssClass", e.g.: { formatter: Formatters.iconBoolean, params: { cssClass: "mdi mdi-check" }}'
    );
  }
  ...
  if (
    value === true ||
    (isNumber && parseFloat(value) > 0) ||
    (!isNumber && typeof value === 'string' && value.toLowerCase() !== 'false' && value !== '0')
  ) {
    isTruthy = true;
  }
