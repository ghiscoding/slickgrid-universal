...
describe('the Percent Symbol Formatter', () => {
  it('should display an empty string when no value is provided', () => {
    const output = percentFormatter(1, 1, '', {} as Column, {}, {} as GridOption);
    expect(output).toBe('');
  });

  it('should display original string when non-numeric value is provided', () => {
    const output = percentFormatter(1, 1, 'hello', {} as Column, {}, {} as GridOption);
    expect(output).toBe('hello');
  });

  it('should display 0% when number 0 is provided', () => {
    const input = 0;
    const output = percentFormatter(1, 1, input, {} as Column, {}, {} as GridOption);
  ...
  it('should display thousand separated percentage when the thousand separator is provided', () => {
    const input = '345678';
    const output = percentFormatter(1, 1, input, { params: { thousandSeparator: ',' } } as Column, {}, {} as GridOption);
  ...
  it('should display a negative number with parentheses when "displayNegativeNumberWithParentheses" is enabled in the "params"', () => {
    const input = -0.024;
    const output = percentFormatter(1, 1, input, { params: { displayNegativeNumberWithParentheses: true } } as Column, {}, {} as GridOption);
