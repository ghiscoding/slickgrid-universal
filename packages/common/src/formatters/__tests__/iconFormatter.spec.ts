...
describe('the Icon Formatter', () => {
  ...
  it('should show console warning when using deprecated icon/formatterIcon params', () => {
    ...
    const result = iconFormatter(0, 0, input, { field: 'user', params: { icon } } as Column, {}, {} as any);
    expect(result).toBe(`<i class="${icon}" aria-hidden="true"></i>`);
    expect(consoleSpy).toHaveBeenCalledWith('[Slickgrid-Universal] deprecated params.icon or params.formatterIcon are deprecated when using `Formatters.icon` in favor of params.iconCssClass. (e.g.: `{ formatter: Formatters.icon, params: { iconCssClass: "fa fa-search" }}`');
  });
});
