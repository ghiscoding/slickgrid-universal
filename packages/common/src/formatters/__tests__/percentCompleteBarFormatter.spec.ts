...
describe('the Percent Complete Formatter', () => {
  it('should return an empty string when no value is provided', () => {
    const output = percentCompleteBarFormatter(1, 1, '', {} as Column, {}, {} as any);
    expect(output).toBe('');
  });

  it('should return empty string when non-numeric value is provided', () => {
    const output = percentCompleteBarFormatter(1, 1, 'hello', {} as Column, {}, {} as any);
    expect(output).toBe('');
  });
  ...
  it('should display a red color bar formatter when number 0 is provided', () => {
    ...
    expect((output as HTMLElement).outerHTML).toBe(`<span class="percent-complete-bar" title="${input}%" style="background: ${color}; width: ${input}%;"></span>`);
  });
  ...
  it('should display a silver color bar when value is between 30 and 69', () => {
    ...
    const input2 = 69;
    const color = 'silver';
    const output1 = percentCompleteBarFormatter(1, 1, input1, {} as Column, {}, {} as any);
    const output2 = percentCompleteBarFormatter(1, 1, input2, {} as Column, {}, {} as any);
    expect((output1 as HTMLElement).outerHTML).toBe(`<span class="percent-complete-bar" title="${input1}%" style="background: ${color}; width: ${input1}%;"></span>`);
    expect((output2 as HTMLElement).outerHTML).toBe(`<span class="percent-complete-bar" title="${input2}%" style="background: ${color}; width: ${input2}%;"></span>`);
  });
