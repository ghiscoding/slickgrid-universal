  ...
  buildFilterSelectFromCollection(collection: any[]): HTMLSelectElement {
    ...
    if (collection.every(x => typeof x === 'string')) {
      ...
      collection.forEach(option => {
        ...
        const textLabel = ((option.labelKey || isEnabledTranslate) && this.translater !== undefined && this.translater?.getCurrentLanguage?.()) ? this.translater.translate(labelKey || ' ') : labelKey;
