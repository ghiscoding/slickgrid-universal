...
const mockAutocompleteFilter = jest.fn().mockImplementation(() => ({
  constructor: jest.fn(),
  init: jest.fn(),
  destroy: jest.fn(),
} as unknown as AutoCompleteFilter));

describe('Filter Factory', () => {
  jest.mock('../autoCompleteFilter', () => mockAutocompleteFilter);
  const Filters = {
    input: InputFilter,
    autoComplete: mockAutocompleteFilter
  };
