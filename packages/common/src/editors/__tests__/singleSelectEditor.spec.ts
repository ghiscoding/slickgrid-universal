    ...
    describe('enableRenderHtml property', () => {
      it('should create the multi-select editor with a default value and have the HTML rendered when "enableRenderHtml" is set', () => {
        mockColumn.editor = {
          ...
          collection: [{ value: true, label: 'True', labelPrefix: `<i class="sgi sgi-check"></i> ` }, { value: false, label: 'False' }],
          customStructure: {
            value: 'isEffort',
            label: 'label',
        ...
        expect(editorListElm.length).toBe(2);
        expect(editorListElm[0].innerHTML).toBe('<i class="sgi sgi-check"></i> True');
      });
      ...
      it('should create the multi-select editor with a default value and have the HTML rendered and sanitized when "enableRenderHtml" is set and has <script> tag', () => {
        ...
        mockColumn.editor = {
          enableRenderHtml: true,
          collection: [{ isEffort: true, label: 'True', labelPrefix: `<script>alert('test')></script><i class="sgi sgi-check"></i> ` }, { isEffort: false, label: 'False' }],
          collectionOptions: {
        ...
        expect(editor.getValue()).toEqual(`<script>alert('test')></script><i class="sgi sgi-check"></i> : true`);
        expect(editorListElm.length).toBe(2);
        expect(editorListElm[0].innerHTML).toBe('<i class="sgi sgi-check"></i> : True');
      });
    });
