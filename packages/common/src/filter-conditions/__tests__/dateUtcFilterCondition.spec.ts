...
describe('dateUtcFilterCondition method', () => {
  it('should return False when no cell value is provided, neither search terms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', cellValue: '', fieldType: 'dateUtc' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });

  it('should return False when any cell value is provided without any search terms', () => {
    const searchTerms = undefined;
    const options = { dataKey: '', operator: 'EQ', cellValue: '2000-12-25T23:01:52.103Z', fieldType: 'dateUtc' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when search term is not a valid date', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', cellValue: '2000-12-25T23:01:52.103Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
  ...
  it('should return True when input value provided is equal to the searchTerms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', cellValue: '1993-12-25T10:50:50.124Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
  ...
  it('should return False when cell value is not the same value as the searchTerm', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', cellValue: '1993-12-25T10:50:50.108Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
  ...
  it('should return False even when Operator is Not Equal and cell value equals the search term', () => {
    ...
    const options = { dataKey: '', operator: 'NE', cellValue: '1993-12-25T10:50:50.108Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
  ...
  it('should return True even when Operator is Not Equal and cell value is different than the search term', () => {
    ...
    const options = { dataKey: '', operator: 'NE', cellValue: '1993-12-25T10:50:50.115Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(true);
  });
  ...
  it('should return False when there are no search term and no operator', () => {
    ...
    const options = { dataKey: '', cellValue: '1993-12-25T10:50:50.108Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when input value is not in the range of search terms', () => {
    const searchTerms = ['1993-12-01T10:22:33.128Z..1993-12-31T12:27:27.127Z'];
    const options = { dataKey: '', operator: 'EQ', cellValue: '1993-11-25T11:50:50.108Z', fieldType: 'dateUtc', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUtc'));
    expect(output).toBe(false);
  });
