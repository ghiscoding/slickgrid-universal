...
describe('dateEuroShortFilterCondition method', () => {
  it('should return False when no cell value is provided, neither search terms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateEuroShort', cellValue: '' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
    expect(output).toBe(false);
  });

  it('should return False when any cell value is provided without any search terms', () => {
    const searchTerms = undefined;
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateEuroShort', cellValue: '25/12/00' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when search term is not a valid date', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateEuroShort', cellValue: '25/12/00', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return True when input value provided is equal to the searchTerms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateEuroShort', cellValue: '25/12/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
  ...
  it('should return False even when Operator is Not Equal and cell value equals the search term', () => {
    const searchTerms = ['25/12/93'];
    const options = { dataKey: '', operator: 'NE', fieldType: 'dateEuroShort', cellValue: '25/12/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when there are no search term and no operator', () => {
    ...
    const options = { dataKey: '', fieldType: 'dateEuroShort', cellValue: '25/12/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
    expect(output).toBe(false);
  });
  ...
  describe('without zero padding dates', () => {
    it('should return True when input value is in the range of search terms with short US dates and without zero padding', () => {
      ...
      const options = { dataKey: '', operator: 'EQ', cellValue: '25/12/93', fieldType: 'dateEuroShort', searchTerms } as FilterConditionOption;
      const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
      expect(output).toBe(true);
    });
    ...
    it('should return False when input value is not in the range of search terms with short US dates and without zero padding', () => {
      const searchTerms = ['1/12/93..31/12/93'];
      const options = { dataKey: '', operator: 'EQ', cellValue: '25/11/93', fieldType: 'dateEuroShort', searchTerms } as FilterConditionOption;
      const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateEuroShort'));
      expect(output).toBe(false);
    });
