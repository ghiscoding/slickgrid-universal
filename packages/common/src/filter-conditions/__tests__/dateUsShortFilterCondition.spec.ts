...
describe('dateUsShortFilterCondition method', () => {
  it('should return False when no cell value is provided, neither search terms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateUsShort', cellValue: '' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });

  it('should return False when any cell value is provided without any search terms', () => {
    const searchTerms = undefined;
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateUsShort', cellValue: '12/25/00' } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when search term is not a valid date', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateUsShort', cellValue: '12/25/00', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return True when input value provided is equal to the searchTerms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', fieldType: 'dateUsShort', cellValue: '12/25/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
  ...
  it('should return False even when Operator is Not Equal and cell value equals the search term', () => {
    const searchTerms = ['12/25/93'];
    const options = { dataKey: '', operator: 'NE', fieldType: 'dateUsShort', cellValue: '12/25/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when there are no search term and no operator', () => {
    ...
    const options = { dataKey: '', fieldType: 'dateUsShort', cellValue: '12/25/93', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return True when input value is in the range of search terms', () => {
    ...
    const options = { dataKey: '', operator: 'EQ', cellValue: '12/25/93', fieldType: 'dateUsShort', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(true);
  });
  ...
  it('should return False when input value is not in the range of search terms', () => {
    const searchTerms = ['12/1/93..12/31/93'];
    const options = { dataKey: '', operator: 'EQ', cellValue: '11/25/93', fieldType: 'dateUsShort', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return True when input value equals the search terms min inclusive value and operator is set to "rangeInclusive"', () => {
    const searchTerms = ['12/1/93..12/31/93'];
    const options = { dataKey: '', operator: 'RangeInclusive', cellValue: '12/1/93', fieldType: 'dateUsShort', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(true);
  });
  ...
  it('should return False when input value equals the search terms min inclusive value and operator is set to "RangeExclusive"', () => {
    const searchTerms = ['12/1/93..12/31/93'];
    const options = { dataKey: '', operator: 'RangeExclusive', cellValue: '12/1/93', fieldType: 'dateUsShort', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
  ...
  it('should return False when any of the 2 search term value is not a valid date', () => {
    ...
    const options = { dataKey: '', operator: 'RangeExclusive', cellValue: '12/5/93', fieldType: 'dateUsShort', searchTerms } as FilterConditionOption;
    const output = executeFilterConditionTest(options, getFilterParsedDates(searchTerms, 'dateUsShort'));
    expect(output).toBe(false);
  });
});
