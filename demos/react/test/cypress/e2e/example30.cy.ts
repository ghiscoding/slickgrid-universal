...
describe('Example 30  Composite Editor Modal', () => {
  ...
  it('should display 2 different tooltips when hovering icons on "Title" column', () => {
    ...
    cy.get('@title-column').find('.mdi-information-outline').trigger('mouseover', { force: true });
  ...
  it('should have "TASK 0" (uppercase) incremented by 1 after each row', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(1)')
      .contains('TASK 0', { matchCase: false })
      .should('have.css', 'text-transform', 'uppercase');
    cy.get('[data-row="1"] > .slick-cell:nth(1)').contains('TASK 1', { matchCase: false });
    cy.get('[data-row="2"] > .slick-cell:nth(1)').contains('TASK 2', { matchCase: false });
    cy.get('[data-row="3"] > .slick-cell:nth(1)').contains('TASK 3', { matchCase: false });
    cy.get('[data-row="4"] > .slick-cell:nth(1)').contains('TASK 4', { matchCase: false });
    cy.get('[data-row="5"] > .slick-cell:nth(1)').contains('TASK 5', { matchCase: false });
  });

  it('should be able to change "Duration" values of first 4 rows', () => {
    // change duration
    cy.get('[data-row="0"] > .slick-cell:nth(2)').should('contain', 'days').click();
    cy.get('.editor-duration').type('0{enter}');
    cy.get('[data-row="0"] > .slick-cell:nth(2)')
      .should('contain', '0 day')
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);

    cy.get('[data-row="1"] > .slick-cell:nth(2)').click().type('1{enter}');
    cy.get('[data-row="1"] > .slick-cell:nth(2)')
      .should('contain', '1 day')
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);

    cy.get('[data-row="2"] > .slick-cell:nth(2)').click().type('2{enter}');
    cy.get('[data-row="2"] > .slick-cell:nth(2)')
  ...
  it('should be able to change "Title" values of row indexes 1-3', () => {
    // change title
    cy.get('[data-row="1"] > .slick-cell:nth(1)').contains('TASK 1', { matchCase: false }).click();
    cy.get('.editor-title').type('task 1111');
    cy.get('.editor-title .editor-footer .btn-save').click();
    cy.get('[data-row="1"] > .slick-cell:nth(1)')
      .contains('TASK 1111', { matchCase: false })
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);

    cy.get('[data-row="2"] > .slick-cell:nth(1)').contains('TASK 2', { matchCase: false }).click();
    cy.get('.editor-title').type('task 2222');
    cy.get('.editor-title .editor-footer .btn-save').click();
    cy.get('[data-row="2"] > .slick-cell:nth(1)')
  ...
  it('should not be able to change the "Finish" dates on first 2 rows', () => {
    cy.get('[data-row="1"] > .slick-cell:nth(8)').should('contain', '').click({ force: true }); // this date should also always be initially empty
    cy.get('[data-vc-date-today]:visible button').should('not.exist');

    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('contain', '').click({ force: true }); // this date should also always be initially empty
    cy.get('[data-vc-date-today]:visible button').should('not.exist');
  });
  ...
  it('should be able to change "Completed" values of row indexes 2-4', () => {
    ...
    cy.get('[data-row="1"] > .slick-cell:nth(7)').click();
    cy.get('.editor-completed').check();

    cy.get('[data-row="2"] > .slick-cell:nth(7)').click();
    cy.get('.editor-completed').check();
  });
  ...
  it('should be able to change "Finish" values of row indexes 0-2', () => {
    ...
    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('contain', '').click(); // this date should also always be initially empty
    cy.get('[data-vc-date-today]:visible button').click('bottom', { force: true });
    cy.get('[data-row="0"] > .slick-cell:nth(8)')
      .should('contain', `${zeroPadding(currentMonth)}/${zeroPadding(currentDate)}/${currentYear}`)
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);

    cy.get('[data-row="1"] > .slick-cell:nth(8)').click();
    cy.get('[data-vc-date-today]:visible button').click('bottom', { force: true });
    cy.get('[data-row="1"] > .slick-cell:nth(8)')
      .should('contain', `${zeroPadding(currentMonth)}/${zeroPadding(currentDate)}/${currentYear}`)
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);

    cy.get('[data-row="2"] > .slick-cell:nth(8)').click();
    cy.get('[data-vc-date-today]:visible button').click('bottom', { force: true });
    cy.get('[data-row="2"] > .slick-cell:nth(8)')
      .should('contain', `${zeroPadding(currentMonth)}/${zeroPadding(currentDate)}/${currentYear}`)
      .should('have.css', 'background-color')
      .and('eq', UNSAVED_RGB_COLOR);
  ...
  it('should have new TASK 8888 displayed on first row', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(1)').contains('TASK 8888', { matchCase: false });
    cy.get('[data-row="0"] > .slick-cell:nth(2)').should('contain', '22 days');
    cy.get('[data-row="0"] > .slick-cell:nth(4)').should('contain', '5');
    cy.get('[data-row="0"] > .slick-cell:nth(5).editable-field').should('have.length', 1);
    cy.get('[data-row="0"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="0"] > .slick-cell:nth(9)').should('contain', 'Tasty Granite Table');

    // next few rows Title should be unchanged
    cy.get('[data-row="1"] > .slick-cell:nth(1)').contains('TASK 0', { matchCase: false });
    cy.get('[data-row="2"] > .slick-cell:nth(1)').contains('TASK 1111', { matchCase: false });
  });
  ...
  it('should open the Composite Editor (Edit Item) and expect all form inputs to be filled with TASK 8888 data of previous create item', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(3)').click({ force: true });
  ...
  it('should have new TASK 8899 displayed on first row', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(1)').contains('TASK 8899', { matchCase: false });
    cy.get('[data-row="0"] > .slick-cell:nth(2)').should('contain', '33 days');
    cy.get('[data-row="0"] > .slick-cell:nth(4)').should('contain', '17');
    cy.get('[data-row="0"] > .slick-cell:nth(5).editable-field').should('have.length', 1);
    cy.get('[data-row="0"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('not.exist');
    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('be.empty');
    cy.get('[data-row="0"] > .slick-cell:nth(9)').should('contain', 'Tasty Granite Table');
  ...
  it('should open the Composite Editor (Mass Update) and be able to change some of the inputs in the form', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(3)').click();
  ...
  it('should have updated values in the entire grid', () => {
    cy.get('[data-row="0"] > .slick-cell:nth(4)').should('contain', '51');
    cy.get('[data-row="0"] > .slick-cell:nth(5)').should('contain', 'Straightforward');
    cy.get('[data-row="0"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="0"] > .slick-cell:nth(10)').should('contain', 'Belgium');

    cy.get('[data-row="1"] > .slick-cell:nth(4)').should('contain', '51');
    cy.get('[data-row="0"] > .slick-cell:nth(5)').should('contain', 'Straightforward');
    cy.get('[data-row="1"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="1"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="1"] > .slick-cell:nth(10)').should('contain', 'Belgium');

    cy.get('[data-row="2"] > .slick-cell:nth(4)').should('contain', '51');
    cy.get('[data-row="0"] > .slick-cell:nth(5)').should('contain', 'Straightforward');
    cy.get('[data-row="2"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="2"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="2"] > .slick-cell:nth(10)').should('contain', 'Belgium');

    cy.get('[data-row="3"] > .slick-cell:nth(4)').should('contain', '51');
    cy.get('[data-row="0"] > .slick-cell:nth(5)').should('contain', 'Straightforward');
    cy.get('[data-row="3"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="3"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="3"] > .slick-cell:nth(10)').should('contain', 'Belgium');
  });
  ...
  it('should select row 1 and 2', () => {
    cy.get('[data-row="1"] > .slick-cell:nth(0)').click();
  ...
  it('should have updated all the changed values BUT only on the 2 selected rows', () => {
    ...
    cy.get('[data-row="1"] > .slick-cell:nth(4)').should('contain', '77');
    cy.get('[data-row="1"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="1"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="1"] > .slick-cell:nth(10)').should('contain', 'Belize');

    cy.get('[data-row="2"] > .slick-cell:nth(4)').should('contain', '77');
    cy.get('[data-row="2"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="2"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="2"] > .slick-cell:nth(10)').should('contain', 'Belize');
  ...
  it('should change the "Title" & "Duration" from the Clone form, then click on "Clone" button and expect a new row to show up on top of the grid', () => {
    ...
    cy.get('[data-row="0"] > .slick-cell:nth(1)').contains('TASK 9999', { matchCase: false });
    cy.get('[data-row="0"] > .slick-cell:nth(2)').should('contain', '44 days');
    cy.get('[data-row="0"] > .slick-cell:nth(4)').should('contain', '17');
    cy.get('[data-row="0"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 0);
    cy.get('[data-row="0"] > .slick-cell:nth(8)').should('be.empty');
    cy.get('[data-row="0"] > .slick-cell:nth(10)').should('contain', 'Belgium');
  });

  it('should expect original, that was originally used to clone, to now be exist as that 2nd row in the grid', () => {
    cy.get('[data-row="1"] > .slick-cell:nth(1)').should('contain', '8899');
    cy.get('[data-row="1"] > .slick-cell:nth(2)').should('contain', '33 days');
    cy.get('[data-row="1"] > .slick-cell:nth(4)').should('contain', '51');
    cy.get('[data-row="1"] > .slick-cell:nth(7)').find('.mdi.mdi-check.checkmark-icon').should('have.length', 1);
    cy.get('[data-row="1"] > .slick-cell:nth(8)').should('not.be.empty');
    cy.get('[data-row="1"] > .slick-cell:nth(10)').should('contain', 'Belgium');
  ...
  it('should be able to clear the "Country of Origin" autocomplete field in the grid via the Clear button from the editor', () => {
    ...
    // clear Country
    cy.get('[data-row="1"] > .slick-cell:nth(10)').click();
    cy.get('.autocomplete-container button.btn-clear').click();
    cy.get('[data-row="1"] > .slick-cell:nth(10)').should('contain', '');
  });
  ...
  it('should open Clone Composite Editor from Cell Menu and expect Task 4 on 6th row', () => {
    cy.get('[data-row="6"] > .slick-cell:nth(10)').click();
    cy.get('[data-row="6"] > .slick-cell:nth(11)').click({ force: true });
