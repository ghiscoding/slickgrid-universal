...
describe('Example 15: Grid State & Presets using Local Storage', () => {
  ...
  it('should have Pagination displayed with default values', () => {
    ...
    cy.get('@grid15').find('[data-test=page-count]').contains('20');

    cy.get('@grid15').find('[data-test=item-from]').contains('1');

    cy.get('@grid15').find('[data-test=item-to]').contains('25');

    cy.get('@grid15').find('[data-test=total-items]').contains('500');
  });
  ...
  it('should hide the "Start" column from the Column Picker', () => {
    ...
    cy.get('#grid15').find('.slick-header-column').first().trigger('mouseover').trigger('contextmenu').invoke('show');
  ...
  it('should select row (Task 105)', () => {
    cy.get('#grid15').contains('Task 105').parent().children('.slick-cell-checkboxsel').find('input[type=checkbox]').click({ force: true });
  });
  ...
  it('should change Page Size and Page Number then expect the Pagination to have correct values', () => {
    ...
    cy.get('@grid15').find('#items-per-page-label').select('20');
  ...
  it('should go back to first page and expect row selection (Task 105) to be persisted', () => {
    ...
    cy.get('@grid15').find('.icon-seek-first').click().wait(10);
