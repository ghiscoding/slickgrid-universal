...
const Example58: React.FC = () => {
  const [columns, setColumns] = useState<Column[]>([]);
  const [dataset, setDataset] = useState<FinancialRow[]>([]);
  const [gridOptions, setGridOptions] = useState<GridOption | undefined>(undefined);
  const [subTitleStyle, setSubTitleStyle] = useState('display: block');
  ...
  function toggleSubTitle() {
    setSubTitleStyle((style) => (style === 'display: block' ? 'display: none' : 'display: block'));
  ...
  return !gridOptions ? null : (
    <div id="demo-container" className="container-fluid">
      <h2>
        ...
        <span className="float-end">
          <a
            style={{ fontSize: '18px' }}
            target="_blank"
            href="https://github.com/ghiscoding/slickgrid-universal/blob/master/demos/react/src/examples/slickgrid/Example58.tsx"
          >
      ...
      <h6 className="italic content example-details" style={{ display: subTitleStyle === 'display: block' ? 'block' : 'none' }}>
        ...
        scrolling would clip them. Total Revenue, Total Expenses, and Net Profit also use two-sided stickiness and dock to whichever
        vertical edge is closest after they have been seen.
      </h6>
      ...
      <div className="financial-report-legend">
        <span>
          <i className="financial-report-legend-swatch financial-report-legend-account"></i> left sticky column
        </span>
        <span>
          <i className="financial-report-legend-swatch financial-report-legend-total"></i> bottom sticky row
        </span>
        <span>
          <i className="financial-report-legend-swatch financial-report-legend-intersection"></i> sticky intersection
        </span>
        <span>
          <i className="financial-report-legend-swatch financial-report-legend-sticky"></i> sticky quarter
        </span>
      </div>
    </div>
  );
