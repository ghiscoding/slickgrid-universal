...
export class Example28 {
  ...
  treeFormatter: Formatter = (_row, _cell, value, _columnDef, dataContext, gridOptions) => {
    ...
    const data = this.aureliaGrid.dataView.getItems();
    const identifierPropName = this.aureliaGrid.dataView.getIdPropertyName() || 'id';
    const idx = this.aureliaGrid.dataView.getIdxById(dataContext[identifierPropName]) as number;
