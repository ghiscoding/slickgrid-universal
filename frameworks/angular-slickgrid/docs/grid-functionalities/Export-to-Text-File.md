...
If you have lots of data, you might want to show a spinner telling the user that something is happening. You can use the Event Emitters `(onBeforeExportToTextFile)` to start your spinner and then `(onAfterExportToTextFile)` to stop the spinner once the process is done. You can see a this [Grouping Example](https://ghiscoding.github.io/angular-slickgrid-demos/#/grouping) demo which has this feature enabled.
