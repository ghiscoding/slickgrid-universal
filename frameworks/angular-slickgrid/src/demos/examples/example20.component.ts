...
export class Example20Component implements OnInit {
  ...
  angularGridReady(g: AngularGridInstance) {
    this.angularGrid = g;
      ...
      cellMenu: {
        ...
        onOptionSelected: (_e: any, a: any) => {
          if (a?.dataContext) {
            ...
            this.angularGrid?.gridService?.updateItem(a.dataContext);
          }
        },
      },
  ...
  executeCommand(a: any) {
    if (a.command === 'delete-row' && confirm(`Do you really want to delete row (${a.row + 1}) with "${a.dataContext.title}"?`)) {
      this.angularGrid?.gridService?.deleteItemById(a.dataContext.id);
  ...
  setLargePinnedColumns() {
    this.angularGrid?.gridStateService?.applyColumnLayout?.(
      [
