...
describe('Example 23 - Range Filters', () => {
  ...
  describe('Set Dymamic Filters', () => {
    ...
    it('should change dynamic filters from the select and choose "Current Year Completed Tasks" and expect 2 filters set', () => {
      cy.get('[data-test=select-dynamic-filter]').select('Current Year Completed Tasks');
    ...
    it('should clear all filters and filter Description using only "=" and expect only rows with empty description to show up', () => {
      ...
      cy.get('.search-filter[data-columnid=description]').type('=');

      cy.get(`#grid23 [data-row="0"] > .slick-cell:nth(1)`).should('be.empty', '');
      cy.get(`#grid23 [data-row="1"] > .slick-cell:nth(1)`).should('be.empty', '');
      cy.get(`#grid23 [data-row="2"] > .slick-cell:nth(1)`).should('be.empty', '');
      cy.get(`#grid23 [data-row="3"] > .slick-cell:nth(1)`).should('be.empty', '');
      cy.get(`#grid23 [data-row="4"] > .slick-cell:nth(1)`).should('be.empty', '');
    });
    ...
    it('should clear all filters and filter Description using only "=" and expect only rows with empty description to show up', () => {
      ...
      cy.get(`#grid23 [data-row="0"] > .slick-cell:nth(1)`).should('not.be.empty');
      cy.get(`#grid23 [data-row="1"] > .slick-cell:nth(1)`).should('not.be.empty');
      cy.get(`#grid23 [data-row="2"] > .slick-cell:nth(1)`).should('not.be.empty');
      cy.get(`#grid23 [data-row="3"] > .slick-cell:nth(1)`).should('not.be.empty');
      cy.get(`#grid23 [data-row="4"] > .slick-cell:nth(1)`).should('not.be.empty');
    });
  });
