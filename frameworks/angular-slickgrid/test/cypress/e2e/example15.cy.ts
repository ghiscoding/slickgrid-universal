describe('Example 15 - Column Span & Header Grouping', () => {
  ...
  it('should have a frozen grid on page load with 3 columns on the left and 4 columns on the right', () => {
    cy.get('#grid2').find('[style="transform: translateY(0px);"]').should('have.length', 2);
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"]').children().should('have.length', 3);
    cy.get('#grid2').find('.grid-canvas-right > [style="transform: translateY(0px);"]').children().should('have.length', 4);

    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(0)').should('contain', '0');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(1)').should('contain', 'Task 0');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(2)').should('contain', '5 days');

    cy.get('#grid2')
      .find('.grid-canvas-right > [style="transform: translateY(0px);"] > .slick-cell:nth(0)')
      .should('contain', '01/01/2009');
    cy.get('#grid2')
      .find('.grid-canvas-right > [style="transform: translateY(0px);"] > .slick-cell:nth(1)')
  ...
  it('should click on the "Remove Frozen Columns" button to switch to a regular grid without frozen columns and expect 7 columns on the left container', () => {
    ...
    cy.get('#grid2').find('[style="transform: translateY(0px);"]').should('have.length', 1);
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"]').children().should('have.length', 7);

    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(0)').should('contain', '0');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(1)').should('contain', 'Task 0');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(2)').should('contain', '5 days');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(3)').should('contain', '01/01/2009');
    cy.get('#grid2').find('.grid-canvas-left > [style="transform: translateY(0px);"] > .slick-cell:nth(4)').should('contain', '01/05/2009');
  });
