  ...
  describe('drag & drop column reordering with auto-scroll', () => {
    ...
    it('should follow a center column that is resized past the viewport edge', () => {
      ...
      cy.get(`${headerSelector} .slick-resizable-handle`).then(($handle) => {
        ...
        handle.dispatchEvent(new win.MouseEvent('mousedown', { bubbles: true, button: 0, buttons: 1, clientX: startX, clientY: startY }));
        win.document.body.dispatchEvent(
          new win.MouseEvent('mousemove', { bubbles: true, buttons: 1, clientX: win.innerWidth + 800, clientY: startY })
        );
      });
      ...
      cy.window().then((win) => win.document.body.dispatchEvent(new win.MouseEvent('mouseup', { bubbles: true, button: 0 })));
    });
