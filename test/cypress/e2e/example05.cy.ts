...
describe('Example 05 - Tree Data (from a flat dataset with parentId references)', () => {
  ...
  it('should expect all rows to be collapsed on first page load', () => {
    cy.get('.grid5').find('.slick-group-toggle.expanded').should('have.length', 0);
  ...
  it('should expand all rows from "Expand All" button', () => {
    ...
    cy.get('.grid5').find('.slick-group-toggle.collapsed').should('have.length', 0);
  ...
  it('should collapsed all rows from "Collapse All" context menu', () => {
    cy.get('.grid5').contains('5 days');

    cy.get('.grid5').find('.slick-row .slick-cell:nth(1)').rightclick({ force: true });
  ...
  it('should have data filtered, with "% Complete" >=25, and not show the full item count in the footer', () => {
    ...
    cy.get('.item-count').should(($span) => expect($span.text()).not.to.eq('500'));

    cy.get('.total-count').should(($span) => expect($span.text()).to.eq('500'));
  });
  ...
  it('should open the Grid Menu "Clear all Filters" command', () => {
    ...
    cy.get('.grid5 .slickgrid-container')
      ...
      .then(() => {
        cy.get(`.slick-grid-menu.${gridUid}`).find('.slick-menu-item:nth(0)').find('span').contains('Clear all Filters').click();
      });
  });
  ...
  it('should no longer have filters and it should show the full item count in the footer', () => {
    ...
    cy.get('.item-count').should(($span) => expect($span.text()).to.eq('500'));
  ...
  it('should open the Grid Menu "Clear all Filters" command a second time', () => {
    cy.get('.grid5').find('button.slick-grid-menu-button').trigger('click').click();
