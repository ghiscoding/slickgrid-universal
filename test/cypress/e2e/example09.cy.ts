...
describe('Example 09 - OData Grid', () => {
  ...
  describe('when "enableCount" is set', () => {
    ...
    it('should Clear all Sorting', () => {
      cy.get('.grid9').find('button.slick-grid-menu-button').trigger('click').click();
    ...
    it('should use "substringof" when OData version is set to 2', () => {
      ...
      cy.get('.grid9').find('.slick-row').should('have.length', 1);
    });
    ...
    it('should click on Set Dynamic Filter and expect query and filters to be changed', () => {
      ...
      cy.get('.grid9').find('.slick-row').should('have.length', 5);
    });
  ...
  describe('Set Dynamic Sorting', () => {
    ...
    it('should expect the grid to be sorted by "Name" descending', () => {
      cy.get('.grid9').get('.slick-header-column:nth(1)').find('.slick-sort-indicator-desc').should('have.length', 1);
  ...
  describe('Local Storage Persistance', () => {
    it('should persist sorting and re-apply on refresh', () => {
      cy.get('[data-test=clear-local-storage]').click();

      cy.get('.slick-header-columns').children('.slick-header-column:nth(1)').click();

      cy.reload();

      cy.window().its('localStorage').invoke('getItem', STORAGE_KEY).should('not.be.null');
      ...
      cy.window().its('localStorage').invoke('getItem', STORAGE_KEY).should('be.null');
    });
