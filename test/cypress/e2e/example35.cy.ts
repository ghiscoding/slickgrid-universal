describe('Example 35 - Tree Data with Lazy Loading', () => {
  ...
  it('should display Example title', () => {
    ...
    cy.get('h3').should('contain', 'Example 35 - Tree Data with Lazy Loading');
  ...
  it('should expect all folders to be collapsed and expect 6 initial items (6x chapters with children) in the grid', () => {
    ...
    defaultGridPreset.forEach((_colName, rowIdx) => {
      if (rowIdx < defaultGridPreset.length - 1) {
        cy.get(`.grid35 [style="transform: translateY(${GRID_ROW_HEIGHT * rowIdx}px);"] > .slick-cell:nth(1)`).should(
          'contain',
