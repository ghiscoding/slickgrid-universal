describe('Example 42 - Custom Filter Bar', () => {
  ...
  it('should display Example title', () => {
    ...
    cy.get('h3').should('contain', 'Example 42 - Custom Filter Bar');
  });
  ...
  it('should use flex header layout while keeping the configured single title row', () => {
    cy.get('.grid42 .slick-header-columns').should('have.css', 'display', 'flex');
    cy.get('.grid42 .slick-header-column').first().should('have.css', 'float', 'none');
    cy.get('.grid42 .slick-column-name').first().should('have.css', '-webkit-line-clamp', '1');
  });
