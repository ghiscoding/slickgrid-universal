describe('Example 15 - OData Grid using RxJS', () => {
  ...
  describe('when "enableCount" is set', () => {
    ...
    it('should Clear all Filters and expect to go back to first page', () => {
      cy.get('.grid15').find('button.slick-grid-menu-button').trigger('click').click({ force: true });
    ...
    it('should use "substringof" when OData version is set to 2', () => {
      ...
      cy.get('.grid15').find('.slick-row').should('have.length', 1);
    });
    ...
    it('should click on Set Dynamic Filter and expect query and filters to be changed', () => {
      cy.get('[data-test=set-dynamic-filter-btn]').click();
      ...
      cy.get('.grid15').find('.slick-row').should('have.length', 5);
    });
  });
  ...
  describe('Set Dynamic Sorting', () => {
    it('should click on "Set Filters Dynamically" then on "Set Sorting Dynamically"', () => {
      ...
      cy.get('[data-test=set-dynamic-sorting-btn]').click();
    ...
    it('should expect the grid to be sorted by "Name" descending', () => {
      cy.get('.grid15').get('.slick-header-column:nth(1)').find('.slick-sort-indicator-desc').should('have.length', 1);
  ...
  describe('Editors & Filters with RxJS Observable', () => {
    it('should open the "Gender" filter and expect to find 3 options in its list ([blank], male, female)', () => {
      ...
      cy.get('[data-name="filter-gender"].ms-drop').find('li:visible').should('have.length', 3);
    ...
    it('should select "male" Gender and expect only 4 rows left in the grid', () => {
      cy.get('[data-name="filter-gender"].ms-drop').find('li:visible:nth(1)').contains('male').click();

      cy.get('.grid15').find('.slick-row').should('have.length', 4);
    });
    ...
    it('should be able to open "Gender" on the first row and expect to find 2 options the editor list (male, female) and expect male to be selected', () => {
      ...
      cy.get('[data-name="editor-gender"].ms-drop').find('li:visible').should('have.length', 2);
    ...
    it('should select 1st row', () => {
      ...
      cy.get('.grid15').find('.slick-row').children().filter('.slick-cell-checkboxsel.selected').should('have.length', 1);
    });
    ...
    it('should open the "Gender" editor on the first row and expect to find 1 more option the editor list (male, female, other)', () => {
      ...
      cy.get('[data-name="editor-gender"].ms-drop').find('li:visible').should('have.length', 3);
    ...
    it('should be able to change the Gender editor on the first row to the new option "other"', () => {
      cy.get('[data-name="editor-gender"].ms-drop').find('li:visible:nth(2)').contains('other').click();
    });
    ...
    it('should open Gender filter and now expect to see 1 more option in its list ([blank], male, female, other)', () => {
      ...
      cy.get('[data-name="filter-gender"].ms-drop').find('li:visible').should('have.length', 4);
    ...
    it('should choose "other" form the Gender filter and expect 1 row left in the grid', () => {
      cy.get('[data-name="filter-gender"].ms-drop').find('li:visible:nth(3)').contains('other').click();

      cy.get('.grid15').find('.slick-row').should('have.length', 1);

      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(1)`).should('contain', 'Ayers Hood');
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(2)`).should('contain', 'other');
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(3)`).should('contain', 'Accuprint');
    });
    ...
    it('should mouse over Task 2 cell of last column and expect async tooltip to show up', () => {
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(3)`).as('task2-cell');
      ...
      cy.get('.tooltip-2cols-row:nth(0)')
        .find('div:nth(1)')
        .contains(/\d+ Belleville blvd$/); // use regexp to make sure it's a number

      cy.get('.tooltip-2cols-row:nth(1)').find('div:nth(0)').contains('Zip:');
      cy.get('.tooltip-2cols-row:nth(1)')
        .find('div:nth(1)')
        .contains(/\d{6}$/); // use regexp to make sure it's a number
    ...
    it('should return 2 rows using "C*n" (starts with "C" + ends with "n")', () => {
      ...
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(1)`).should('contain', 'Consuelo Dickson');
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 1}px);"] > .slick-cell:nth(1)`).should('contain', 'Christine Compton');
    });
    ...
    it('should change Gender filter back to "male" with descending order', () => {
      ...
      cy.get('.grid15').find('.slick-row').should('have.length', 10);

      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(1)`).should('contain', 'Woods Key');
    ...
    it('should return 2 rows using different case sensitivity search value "ba*E" (starts with "ba" + ends with "E")', () => {
      cy.get('input.filter-name').clear().type('ba*E');
      ...
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 0}px);"] > .slick-cell:nth(1)`).should('contain', 'Barr Page');
      cy.get(`[style="transform: translateY(${GRID_ROW_HEIGHT * 1}px);"] > .slick-cell:nth(1)`).should('contain', 'Barnett Case');
    });
  });
});
