...
describe('Example 10 - GraphQL Grid', () => {
  ...
  it('should have GraphQL query with defined Grid Presets', () => {
    ...
    cy.get('[data-test=graphql-query-result]').should(($span) => {
      ...
      expect(text).to.eq(
        removeSpaces(`query{users(first:20,offset:20,
  ...
  it('should change Pagination to next page', () => {
    ...
    cy.get('[data-test=graphql-query-result]').should(($span) => {
      const text = removeSpaces($span.text()); // remove all white spaces
      expect(text).to.eq(
        removeSpaces(`query{users(first:20,offset:40,
          orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
          filterBy:[
            {field:"gender",operator:EQ,value:"male"},
            {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
  ...
  it('should change Pagination to last page', () => {
    ...
    cy.get('[data-test=graphql-query-result]').should(($span) => {
      const text = removeSpaces($span.text()); // remove all white spaces
      expect(text).to.eq(
        removeSpaces(`query{users(first:20,offset:80,
          orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
          filterBy:[
            {field:"gender",operator:EQ,value:"male"},
            {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
  ...
  it('should change Pagination to first page using the external button', () => {
    ...
    cy.get('[data-test=graphql-query-result]').should(($span) => {
      const text = removeSpaces($span.text()); // remove all white spaces
      expect(text).to.eq(
        removeSpaces(`query { users (first:20,offset:0,
          orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
          filterBy:[
            {field:"gender",operator:EQ,value:"male"},
            {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
  ...
  describe('Translate by Language', () => {
    ...
    it('should open the Grid Menu and expect all commands be displayed in English', () => {
      cy.get('.grid10').find('button.slick-grid-menu-button').trigger('click');
    ...
    it('should display Pagination in French', () => {
      cy.get('.text-item-per-page').contains('éléments par page');

      cy.get('.text-of').contains('de');

      cy.get('.item-from').contains('1');

      cy.get('.item-to').contains('30');

      cy.get('.total-items').contains('100');

      cy.get('.text-items').contains('éléments');
    });
  ...
  describe('Cursor Pagination', () => {
    ...
    it('should change Pagination to the last page', () => {
      ...
      cy.get('[data-test=graphql-query-result]').should(($span) => {
        const text = removeSpaces($span.text()); // remove all white spaces
        expect(text).to.eq(
          removeSpaces(`query{users(last:20,
            orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
            filterBy:[
              {field:"gender",operator:EQ,value:"male"},
              {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
    ...
    it('should change Pagination to the first page', () => {
      ...
      cy.get('[data-test=graphql-query-result]').should(($span) => {
        const text = removeSpaces($span.text()); // remove all white spaces
        expect(text).to.eq(
          removeSpaces(`query{users(first:20,
            orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
            filterBy:[
              {field:"gender",operator:EQ,value:"male"},
              {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
        ...
        cy.get('.icon-seek-next')
          ...
          .then(() => {
            ...
            cy.get('[data-test=graphql-query-result]').should(($span) => {
              ...
              const text = removeSpaces($span.text()); // remove all white spaces
              expect(text).to.eq(
                removeSpaces(`query{users(first:20,after:"${afterCursor}",
                orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
                filterBy:[
                  {field:"gender",operator:EQ,value:"male"},
                  {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
        ...
        cy.get('.icon-seek-prev')
          ...
          .then(() => {
            ...
            cy.get('[data-test=graphql-query-result]').should(($span) => {
              ...
              const text = removeSpaces($span.text()); // remove all white spaces
              expect(text).to.eq(
                removeSpaces(`query{users(last:20,before:"${beforeCursor}",
                orderBy:[{field:"name",direction:ASC},{field:"company",direction:DESC}],
                filterBy:[
                  {field:"gender",operator:EQ,value:"male"},
                  {field:"name",operator:StartsWith,value:"Joh"},{field:"name",operator:EndsWith,value:"oe"},
  ...
  describe('Grid State Changes', () => {
    ...
    it('should focus on Finish Date Range filter, then type Backspace and expect Finish filter to no longer exists in the list of Filters in Grid State change', () => {
      ...
      cy.window().then((win) => {
        ...
        expect(win.console.log).to.be.calledWith('Grid State changed:: ', {
          newValues: { pageNumber: 1, pageSize: 20 },
          type: 'pagination',
        });
      });
    });
