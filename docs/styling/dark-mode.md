...
> **Note** to support both Light and Dark Mode in the same Theme, we use CSS variables **only**. This mean that we **do not** have any dedicated SASS variables for Dark Mode, if you want to override some colors for Dark Mode, you'll need to override the CSS variable for that to work. The best way to find associated variable is to use your browser's debugger tool.